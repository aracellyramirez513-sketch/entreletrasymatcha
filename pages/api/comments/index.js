...
const TIPOS_VALIDOS = ['resena', 'orden', 'universo', 'rincon', 'vineta'];
...
export default async function handler(req, res) {
  // ---------- LEER ----------
  if (req.method === 'GET') {
    const { slug, page_type } = req.query;

    if (!slug || typeof slug !== 'string') {
      return res.status(400).json({ error: 'Falta el parámetro slug' });
    }

    const tipo = TIPOS_VALIDOS.includes(page_type) ? page_type : 'resena';
  ...
  if (req.method === 'POST') {
    ...
    const { slug, page_type, author_name, content, parent_id, website } = req.body || {};
