...
export async function getContextoSerie(libro) {
  if (!libro || !libro.serie) return CONTEXTO_VACIO

  const clave = claveSerie(libro.serie)
  if (!clave) return CONTEXTO_VACIO
  ...
  if (orden && Array.isArray(orden.libros_serie) && orden.libros_serie.length > 0) {
    ...
    libros = todos
      ...
      .map(l => ({
        numero:  Number(l.numero_serie) || null,
        titulo:  l.titulo || '',
        portada: l.portada || '',
        slug:    l.slug,
      }))
  ...
  let idx = libros.findIndex(l => l.slug && l.slug === libro.slug)

  if (idx === -1 && libro.numero_serie !== '' && libro.numero_serie != null) {
    idx = libros.findIndex(l => l.numero === Number(libro.numero_serie))
  }
  if (idx === -1) {
  ...
  if (idx === -1) {
    const propio = {
      numero:  Number(libro.numero_serie) || null,
      titulo:  libro.titulo || '',
      portada: libro.portada || '',
      slug:    libro.slug,
    }
    if (propio.numero) {
      const pos = libros.findIndex(l => (l.numero || 0) > propio.numero)
  ...
  const posicion = libros[idx]?.numero || idx + 1
  const total = Math.max(libros.length, posicion, Number(orden?.num_libros) || 0)

  return {
    orden: orden ? { titulo: orden.titulo, slug: orden.slug } : null,
